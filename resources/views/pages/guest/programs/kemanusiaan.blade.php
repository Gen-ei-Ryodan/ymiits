...
@section('title', 'Program Kemanusiaan')

@section('content')
<section class="bg-gradient-to-b from-white to-green-100 text-black">
    <div class="container mx-auto px-4 md:px-16 py-8 md:py-16" data-aos="fade-up">
        <div class="mb-12 md:mb-20 text-center pt-10">
            <h1 class="text-3xl md:text-5xl font-bold text-gray-800 mb-4 md:mb-6">
                Program <span class="text-green-600">Kemanusiaan</span>
            </h1>
            <p class="max-w-3xl mx-auto text-base md:text-xl text-gray-600 leading-relaxed">
                {{ $programKemanusiaan->deskripsi ?? 'Deskripsi belum tersedia.' }}
            </p>
        </div>

        @if($programKemanusiaan->subPrograms->count() > 0)
            @foreach($programKemanusiaan->subPrograms as $index => $subProgram)
                <div class="flex flex-col md:flex-{{ $index % 2 == 0 ? 'row' : 'row-reverse' }} items-center justify-between py-8 md:py-16 md:space-x-{{ $index % 2 == 0 ? '' : 'reverse ' }}md:space-x-12"
                    ...
                    <div class="w-full md:w-1/2 mb-6 md:mb-0">
                        @if ($subProgram->foto)
                            <img src="{{ asset('storage/program/kemanusiaan/sub/' . $subProgram->foto) }}" alt="{{ $subProgram->judul }}"
            ...
            @if($programKemanusiaan)
                ...
                <div class="space-y-16">
                    @for($i = 1; $i <= 2; $i++)
                        ...
                        @if($programKemanusiaan->$judulField)
                        <div class="flex flex-col md:flex-row items-center md:space-x-12 md:py-8" data-aos="fade-right">
                            @if($programKemanusiaan->$fotoField)
                            <div class="w-full md:w-1/2 mb-6 md:mb-0">
                                <img src="{{ asset('storage/program/kemanusiaan/'.$programKemanusiaan->$fotoField) }}" 
                                    alt="{{ $programKemanusiaan->$judulField }}"
                                    class="w-full h-64 md:h-96 object-cover rounded-xl shadow-lg" />
                            </div>
                            @endif
                            <div class="w-full md:w-1/2 text-center md:text-left space-y-4">
                                <h2 class="text-2xl md:text-3xl font-bold text-green-800">
                                    {{ $programKemanusiaan->$judulField }}
                                </h2>
                                <p class="text-gray-700 text-base md:text-lg leading-relaxed">
                                    {{ $programKemanusiaan->$deskripsiField }}
                                </p>
                                <a href="https://www.itsedekah.id/"
                                    class="inline-block mt-2 px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition">
                                    Donasi Sekarang
                                </a>
                            </div>
                        </div>
