...
@section('title', 'Sub Program Wakaf')

@section('content')
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <div class="card-title">Sub Program Wakaf</div>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($item->foto)
                            <img src="{{ asset('storage/program/wakaf/sub/' . $item->foto) }}" alt="{{ $item->judul }}" class="img-thumbnail" width="100">
                            @else
                            <span class="badge bg-secondary">Tidak ada foto</span>
                            @endif
                        </td>
                        <td>
                            <a href="{{ route('admin.sub-program-wakaf.edit', $item->id) }}" class="btn btn-sm btn-warning">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('admin.sub-program-wakaf.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus sub program ini?');">
                                @csrf
                                @method('DELETE')
...
<div class="modal fade" id="tambahSubProgramModal" tabindex="-1" aria-labelledby="tambahSubProgramModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="{{ route('admin.sub-program-wakaf.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="tambahSubProgramModalLabel">Tambah Sub Program Wakaf</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="form-group mb-3">
                        <label for="program_wakaf_id" class="form-label">Program Wakaf</label>
                        <select name="program_wakaf_id" id="program_wakaf_id" class="form-select @error('program_wakaf_id') is-invalid @enderror" required>
                            <option value="">Pilih Program</option>
                            @foreach($programs as $program)
                            <option value="{{ $program->id }}">{{ $program->judul }}</option>
                            @endforeach
                        </select>
                        @error('program_wakaf_id')
